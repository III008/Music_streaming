...
<%
	...
	ArrayList<MusicBoardRhVO> rplist = dao.getRp_List(rh_bid);
	dao.getUpdateHits(rh_bid);
%>
		...
		<div class="content_board">
				...
				<div>
					...
					<table class="board_content">
						<tr>
							...
							<td colspan="3">
								<a href="board_update.jsp?rh_bid=<%=rh_bid%>"><button type="button" class="btn_style">수정</button></a>
								<a href="board_delete.jsp?rh_bid=<%=rh_bid%>"><button type="button" class="btn_style_2">삭제</button></a>
							</td>
						</tr>
						<tr>
							<th colspan="6"><%= vo.getRh_btitle() %></th>
						</tr>
						<tr>
							<td>추천</td>
							<td><%= vo.getRh_brec() %></td>
							<td>조회</td>
							<td><%= vo.getRh_bhits() %></td>
							<td>등록일</td>
							<td><%= vo.getRh_bdate() %></td>
						</tr>
						<tr>
							<td colspan="6">
								<p>
									<%= vo.getRh_bcontent().replace("\r\n", "<br>") %><br>
									<% if(vo.getRh_bsfile() != null) { %>
									<img src="http://localhost:9000/Music_streaming/upload/<%=vo.getRh_bsfile()%>" width=50%>
									<% } %>
								</p>
							</td>
						</tr>
			...
			<section class="section_2">
				...
				<form name="boardReplyForm" action="boardReplyProc.jsp" method="post" class="reply_write">
					...
					<input type="hidden" name="rh_bid" value="<%= vo.getRh_bid() %>">
