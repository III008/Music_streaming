...
<%
	...
	ArrayList<MusicBoardRbVO> list = dao.getList(start, end);
%>
		...
		<script>
			$(document).ready(function(){
				...
				jQuery("#ampaginationsm").on('am.pagination.change',function(e){
					$(location).attr('href','http://localhost:9000/Music_streaming/board/board_rb/board_list.jsp?rpage='+e.page);  
					//location.href('이동페이지');
				});
			});
		</script>
			...
			<section class="section_1">
				<div>
					...
					<table class="board">
						...
						<% for(MusicBoardRbVO vo : list){ %>
						<tr>
							<td><%= vo.getRno() %></td>
							<td><a href="board_content.jsp?rb_bid=<%=vo.getRb_bid()%>"><%= vo.getRb_btitle() %></a></td>
