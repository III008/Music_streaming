			...
			<section class="section_1">
				<div>
					...
					<form name="boardUpdateForm" action="boardUpdateProc.jsp" method="post"
						...
						<input type="hidden" name="rb_bid" value="<%=rb_bid%>">
						<ul>
							<li>
								<label>제 목</label>
								<input type="text" name="rb_btitle" value="<%=vo.getRb_btitle()%>">
							</li>
							<li>
								<label>내 용</label>
								<textarea name="rb_bcontent"><%= vo.getRb_bcontent() %></textarea>
							</li>
							<li>
								<label>파일첨부</label>
								<% if(vo.getRb_bfile() != null){ %>
								<input type="file" name="rb_bfile"><span id="fname"><%=vo.getRb_bfile() %></span>
								<% }else{ %>
								<input type="file" name="rb_bfile"><span id="fname">선택된 파일 없음</span>
								<% } %>
							</li>
							<li>
								<button type="submit" class="btn_style">수정완료</button>
								<button type="reset" class="btn_style_2">취소</button>
								<a href="board_content.jsp?rb_bid=<%=rb_bid%>"><button type="button" class="btn_style_2">이전페이지</button></a>
