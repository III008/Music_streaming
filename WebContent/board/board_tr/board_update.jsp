			...
			<section class="section_1">
				<div>
					...
					<form name="boardUpdateForm" action="boardUpdateProc.jsp" method="post"
						...
						<input type="hidden" name="tr_bid" value="<%=tr_bid%>">
						<ul>
							<li>
								<label>제 목</label>
								<input type="text" name="tr_btitle" value="<%=vo.getTr_btitle()%>">
							</li>
							<li>
								<label>내 용</label>
								<textarea name="tr_bcontent"><%= vo.getTr_bcontent() %></textarea>
							</li>
							<li>
								<label>파일첨부</label>
								<% if(vo.getTr_bfile() != null){ %>
								<input type="file" name="tr_bfile"><span id="fname"><%=vo.getTr_bfile() %></span>
								<% }else{ %>
								<input type="file" name="tr_bfile"><span id="fname">선택된 파일 없음</span>
								<% } %>
							</li>
							<li>
								<button type="submit" class="btn_style">수정완료</button>
								<button type="reset" class="btn_style_2">취소</button>
								<a href="board_content.jsp?tr_bid=<%=tr_bid%>"><button type="button" class="btn_style_2">이전페이지</button></a>
